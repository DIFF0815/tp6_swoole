...
class WebSocketClient
{
     ...
     * @return bool
     */
    public function connect($timeout = 1): bool
    {
        if (extension_loaded('swoole'))
        {
     ...
     * @return bool
     */
    function doHandShake($headerBuffer): bool
    {
     ...
     * @return bool
     */
    public function send($data, $type = 'text', $masked = true): bool
    {
        if (empty($data))
        {
    ...
    public function publish($opcode, $data = [])
    {

//        if (function_exists('msgpack_pack')){
//            throw new \Exception("function msgpack_pack not define");
//        }

        if (!empty($data)) {
            $data = msgpack_pack($data);
            $body_len = strlen($data) + 2 + 4;
        } else {
            $body_len = 2 + 4;
        }

        $send_buff = pack('vV', $body_len, intval($opcode));
    ...
    public function unPublish($data)
    {
        ...
        //$d1 = substr($data, $offset, 2);
        //var_dump($d1);
        //$data_len = unpack('v', substr($data, $offset, 2));
        //var_dump($data_len);

        $offset += 2;

        //$d2 = substr($data, $offset, 6);
        //var_dump($d2);
        //$opcode = unpack('V', substr($data, $offset, 6));
        //var_dump($opcode);

        $offset += 4;

        //$d3 = substr($data, $offset);
        //var_dump($d3);

        $res = @msgpack_unpack(substr($data, $offset));
     ...
     * @throws \Exception
     */
    function sendJson($data, $masked = true): bool
    {
     ...
     * @return string
     */
    final protected function createHeader(): string
    {
        $host = $this->host;
     ...
     * @return array
     */
    final protected function parseIncomingRaw($header): array
    {
        $retval = array();
        $content = "";
     ...
     * @return string
     */
    public function generateAlphaNumToken(int $length): string
    {
